...
public:
    ...
    Image( const int w, const int h ) : m_data(w*h, {0,0,0}), m_width(w), m_height(h) {}
    ...
    \endcode
    */
    vector<float>& operator() ( const int x, const int y )
    {
    ...
    vector<float> operator() ( const int x, const int y ) const
    {
        std::size_t offset= y * m_width + x;
        assert(offset < m_data.size());
        return m_data[offset];
